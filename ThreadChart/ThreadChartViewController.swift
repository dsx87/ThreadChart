...
class ThreadChartViewController: UIViewController {
    ...
    @objc func resizeMainView(_ notification:Notification){
    ...
    @objc func calculateThread() {
        ...
        if standard == .iso{
            //thread = ISOThread(diameter: diameter, pitch: pitch, isInternal: isInternal, inTolerance: .h, outTolerance: .g)
            ...
            //thread = UNThread(diameter: diameter, TPI: Int(pitch), isInternal: isInternal, tolerance: .two)
